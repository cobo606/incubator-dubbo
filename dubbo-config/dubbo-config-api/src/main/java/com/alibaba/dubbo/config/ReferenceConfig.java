...
public class ReferenceConfig<T> extends AbstractReferenceConfig {
    ...
    private void init() {
        ...
        // ==================== 1. 从系统属性或配置文件中加载与接口名相对应的配置, 并将解析结果赋值给url字段, 一般是用于点对点调用.
        // 从系统变量中获取与接口名对应的属性值
        ...
        // ==================== 2. 用于检测几个核心配置类是否为空, 为空则尝试从其他配置类中获取.
        if (consumer != null) {
        ...
        // ==================== 4. 用于处理 MethodConfig 实例. 该实例包含了事件通知配置, 比如 onreturn、onthrow、oninvoke 等.
        ...
        // ==================== 5. 主要用于解析服务消费者 ip, 以及调用 createProxy 创建代理对象.
        // 获取服务消费者 ip 地址
    ...
    /**
     * <p> createProxy 这个方法除了创建代理对象外, 还会调用其他方法构建以及合并 Invoker 实例, 主要步骤:
     * <p> 首先根据配置检查是否为本地调用, 若是则调用 InjvmProtocol 的 refer 方法生成 InjvmInvoker 实例.
     * 若不是则读取直联配置项, 或注册中心 url, 并将读取到的 url 存储到 urls 中.
     * 然后, 根据 urls 元素数量进行后续操作. 若 urls 元素数量为1, 则直接通过 Protocol 自适应拓展构建 Invoker 实例接口.
     * 若 urls 元素数量大于1, 即存在多个注册中心或服务直联 url, 此时先根据 url 构建 Invoker.
    ...
    private T createProxy(Map<String, String> map) {
        ...
        if (isInjvm() == null) {
            if (url != null && url.length() > 0) { // if a url is specified, don't do local reference
                // url 配置被指定, 则不做本地引用
                isJvmRefer = false;
            } else // by default, reference local service if there is
            ...
            if (url != null && url.length() > 0) { // user specified URL, could be peer-to-peer address, or register center's address.
                ...
                if (us != null && us.length > 0) {
                    for (String u : us) {
                        ...
                        // 检测 url 协议是否为 registry, 若是表明用户想使用指定的注册中心.
